...
import { Collapse, Button, FormGroup } from 'reactstrap'; //to show numbers on the filter
...
export function PriceFilter(__internal_props) {
  ...
  // PriceFilter component takes in an object prop "updatePriceFilter" that will update the state in Main through "changePrice"
  // PriceFilter component takes in an object prop "filterCallback" which is a callback function that call "handleFilter" in Main to re-render the listings shown
  ...
  // Changing State when user increases/decreases price on thes slider
  const _executer11 = () => {
    updateRange = (event, newValue) => {
      setValue(newValue);

      __internal_props.updatePriceFilter(val); //updates state in props
  ...
  let _el_8 = createText("Max Price");
  ...
  const _executer18 = () => {
    addPropTransaction(__props_transactions, _el_16_instance, "min", 300);
  };

  _executer18();

  const _executer19 = () => {
    addPropTransaction(__props_transactions, _el_16_instance, "step", 100);
  };

  _executer19();

  const _executer20 = () => {
    addPropTransaction(__props_transactions, _el_16_instance, "max", 1800);
  };
